...
{
    public class IPodsContainer
    {
        public IWebElement Root { get; set; }

        public IPodsContainer(IWebElement root)
        {
            this.Root = root;
        }
    ...
    public class iPodsPage : Page
    {
        ...
        public static iPodsPage NavigateTo(IWebDriver driver)
        {
            var iPodsPageInstance = YourAccountPage.NavigateTo(driver);

            var menuItems = iPodsPageInstance.GetProductCategoryMenu();
            Actions action = new Actions(driver);
            action.MoveToElement(menuItems[1]).Perform();
            menuItems[6].Click();

            return PageFactoryExtensions.InitPage<iPodsPage>(driver);
        }

        public List<IPodsContainer> GetProducts()
        {
            var roots = this.ProductsContainer.FindElements(By.ClassName("ipods"));

            List<IPodsContainer> products = new List<IPodsContainer>();

            foreach (var rootElement in roots)
            {
                products.Add(new IPodsContainer(rootElement));
            }
